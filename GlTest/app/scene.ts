...
window.addEventListener("DOMContentLoaded", () => {
    ...
    var createScene = () => {
        ...
        var camera = new BABYLON.FreeCamera("camera1", new BABYLON.Vector3(30, 30, -40), scene);

        // target the camera to scene origin
        camera.setTarget(new BABYLON.Vector3(0, 10, 20));
            ...
            if (fireSpheres.length > 0) {
                for (var i = 0; i < fireSpheres.length; i++) {
                    if (!fireSpheres[i].scored) {
                        if (fireSpheres[i].fireSphere.position.x < netLocation.x + 1 &&
                            fireSpheres[i].fireSphere.position.x > netLocation.x - 1 && 
                            fireSpheres[i].fireSphere.position.y < netLocation.y + 1 &&
                            fireSpheres[i].fireSphere.position.y > netLocation.y - 1 && 
                            fireSpheres[i].fireSphere.position.z < netLocation.z + 1 &&
                            fireSpheres[i].fireSphere.position.z > netLocation.z - 1) {
